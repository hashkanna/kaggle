...
for (var_tmp in 1:ncol(training)) {
  replace(training[,var_tmp], is.na(training[,var_tmp]), median(training[,var_tmp], na.rm=T))
}  
for (var_tmp in 1:ncol(test)) {
  replace(test[,var_tmp], is.na(test[,var_tmp]), median(test[,var_tmp], na.rm=T))
}  
