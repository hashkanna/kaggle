...
#tool=preprocessing.MinMaxScaler()
#tool.fit(train)
#train=tool.transform(train)
#test=tool.transform(test)
...
preds1 = np.argsort(list(model.predict(xgtest)))
...
model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=5)
print("gini score on training set"+str(normalized_gini(labels,model.predict(xgb.DMatrix(train)))))
preds2 = np.argsort(list(model.predict(xgtest)))
...
preds.to_csv('xgboost_rank_benchmark.csv')
